...
// Initialzustand der Spieler und ihrer Steine
...
const LudoLogic = () => {
  const [players, setPlayers] = useState(initialPlayers); // Spieler und deren Steine
  const [currentPlayer, setCurrentPlayer] = useState("red"); // Aktueller Spieler
  const [message, setMessage] = useState("Red starts the game!"); // Nachricht anzeigen
  const [dice, setDice] = useState(null); // Würfelergebnis
  const [isAnimating, setIsAnimating] = useState(false); // Animation aktivieren
